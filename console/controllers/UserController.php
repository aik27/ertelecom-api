...
class UserController extends Controller
{
    public function actionIndex()
    {
        ...
        echo 'yii user/change-token' . PHP_EOL;
    }
    ...
    {
        $username = $this->prompt('Login:', ['required' => true]);
    ...
    public function actionChangeToken()
    {
        $username = $this->prompt('Login:', ['required' => true]);
        $model = $this->findModel($username);
        $model->generateAccessToken();
        if ($this->log($model->save())) {
            $this->stdout('New token: ' . $model->access_token, Console::FG_GREEN, Console::BOLD);
            echo PHP_EOL;
        }
    }
    ...
    {
        if (!$model = User::findOne(['login' => $username])) {
